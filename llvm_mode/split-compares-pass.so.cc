...
bool SplitComparesTransform::simplifyCompares(Module &M) {
  ...
  for (auto &F : M) {
    ...
    for (auto &BB : F) { 
      ...
      if((BB.getName().str().find("normal_basicblock")==std::string::npos)&&BB.getName().str().find("inject")==std::string::npos
        &&BB.getName().str().find("inv_cmp")==std::string::npos&&BB.getName().str().find("sign")==std::string::npos){ 
        continue;
      } 
...
bool SplitComparesTransform::splitCompares(Module &M, unsigned bitw) {
  ...
  errs() << "Replacing " << icomps.size() << " compare\n";
...
bool SplitComparesTransform::runOnModule(Module &M) {
  ...
  errs() << "Split-compare-pass by whlg\n"; 
